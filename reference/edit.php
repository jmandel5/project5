...
if (isset($_POST['submit'])) {
	...
	if (is_numeric($_POST['id'])) {
		...
		if ($firstname == '' || $lastname == '' || $phone == '' || $email == '') {
			...
			$result = mysqli_query($connection, "UPDATE epollack_phonelist SET firstname='$firstname', lastname='$lastname', phone='$phone', email='$email' WHERE id='$id'");
	...
	if (isset($_GET['id']) && is_numeric($_GET['id']) && $_GET['id'] > 0) {
		// query db
		$id = $_GET['id'];
		$result = mysqli_query($connection, "SELECT * FROM epollack_phonelist WHERE id=$id");
